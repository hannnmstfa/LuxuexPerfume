<div class="max-w-screen-xl mx-auto">
    <div class="grid grid-cols-12 gap-4 justify-center items-start space-y-2 lg:space-y-0 p-3">
        <div class="col-span-12 lg:hidden relative">
            @forelse($keranjangs as $item)
                <div
                    class="w-full  border-b border-gray-400 dark:border-gray-600 p-1 font-inter flex justify-start items-center rounded gap-2 relative mb-3  {{ in_array($item->id, $errorStok) ? ' bg-red-200 dark:bg-red-600' : '' }}">
                    <div class="absolute top-0 right-0 text-[0.5rem] bg-gold p-1 rounded-es text-black font-semibold">
                        Stok {{ $item->produks->stok }}
                    </div>
                    <img src="{{ asset($item->produks->path_foto) }}" alt="{{ $item->produks->nama }}"
                        class="w-16 h-24 object-cover rounded" />
                    <div class="flex flex-col justify-center items-start w-full">
                        <p class="font-bold">{{ $item->produks->nama }}</p>
                        <div class="flex justify-between items-center w-full">
                            <p class="text-xs italic text-gray-400">Rp{{ number_format($item->produks->harga)}}</p>
                            <p class="font-semibold text-gray-200">
                                Rp{{ number_format($item->produks->harga * $item->jumlah) }}</p>
                        </div>
                        <div class="flex justify-between items-center w-full mt-5">
                            <button wire:click="hapusKeranjang({{ $item->id }})"
                                class="flex justify-center items-center rounded-full border border-red-600 p-2 hover:opacity-80 {{ in_array($item->id, $errorStok) ? 'dark:bg-red-300' : '' }} "
                            ...
                            <div
                                class=" flex items-center max-w-[9rem] shadow-xs border border-gray-700 bg-gray-900 rounded-full p-1 justify-center gap-1">
                                <button type="button" wire:click="kurangJumlah({{ $item->id }})"
                                    class=" {{ $item->jumlah == 1 ? 'hidden' : '' }} font-medium hover:text-white hover:bg-gray-700 rounded-full text-sm p-1">
                                ...
                                <input type="text" min="1" value="{{ $item->jumlah }}" readonly
                                    class="block h-6 text-center border-none rounded-full w-10 lg:w-full py-2.5 dark:bg-gray-900"
                                    required />
                                <button type="button" wire:click="addJumlah({{ $item->id }})"
                                    class=" font-medium hover:text-white hover:bg-gray-700 rounded-full text-sm p-1">
                                    <svg class="w-4 h-4 text-heading" aria-hidden="true" xmlns="http://www.w3.org/2000/svg"
                                        width="24" height="24" fill="none" viewBox="0 0 24 24">
                                        <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"
                        ...
                        <p class="text-[0.5rem] italic text-end w-full {{ in_array($item->id, $errorStok) ? 'text-red-200' : 'text-red-600' }} {{ $item->produks->stok < $item->jumlah ? 'block' : 'hidden' }}">
                                    Jumlah melebihi stok tersisa</p>
                    </div>
        ...
        <div class="col-span-8 relative overflow-x-auto hidden lg:block">
            <table class="w-full font-inter text-sm text-left text-gray-500 dark:text-white">
                <thead
                    class="text-xs text-gray-700 uppercase bg-yellow-200 dark:bg-gold dark:text-white border-b border-gray-400">
                    <tr>
                    ...
                    @forelse($keranjangs as $item)
                        <tr
                            ...
                            <td class="px-6 py-4">
                                ...
                                <p
                                    class="text-xs {{ in_array($item->id, $errorStok) ? 'text-red-200' : 'text-red-600' }} {{ $item->produks->stok < $item->jumlah ? 'block' : 'hidden' }}">
                                    Jumlah melebihi stok tersisa</p>
                            </td>
        ...
        <div class="col-span-12 lg:col-span-4 sticky top-24">
            <div
                class="border border-gray-300 dark:border-gray-700 rounded-md p-4 shadow-md relative bg-black/10 backdrop-blur">
                <div wire:loading.remove.class="hidden" wire:loading.class="flex"

<x-app-layout title="Users Aktif">
    <div
        ...
        <div class="flex-row items-center justify-between p-4 space-y-3 sm:flex sm:space-y-0 sm:space-x-4">
            <div>
                <h5 class="mr-3 text-xl font-semibold dark:text-white">Kelola Pengguna Aktif</h5>
                <ol class="inline-flex items-center space-x-1 md:space-x-2 rtl:space-x-reverse">
                    ...
                    <li aria-current="page">
                        <div class="flex items-center">
                            ...
                            <span class="ms-1 text-sm font-medium text-gray-500 md:ms-2 dark:text-gray-400">Aktif</span>
                        </div>
                    </li>
                </ol>
            </div>
            ...
            <tbody>
                @foreach ($users as $data)
                    <tr
                        ...
                        <td class="px-6 py-4">
                            <p class="text-xs text-gray-900 dark:text-gray-500 text-nowrap">Registered: <span class="font-semibold dark:text-white">{{ $data->created_at  }}</span></p>
                            <p class="text-xs text-gray-900 dark:text-gray-500 text-nowrap">Verified: <span class="font-semibold dark:text-white">{{ $data->email_verified_at ?? '-' }}</span></p>
                        </td>
                        <td class="px-6 py-4">
                        ...
                        <td class="px-6 py-4">
                            <span
                                class="text-xs font-bold uppercase border rounded-xl py-1 px-3 {{ $data->role == 'admin' ? 'bg-green-100 text-green-500 border-green-400' : 'bg-yellow-100 text-yellow-500 border-gold' }}">{{ $data->role }}</span>
                        </td>
                        <td class="px-6 py-4">
                            <div class="flex items-start justify-start gap-1">
                                <a href="{{ route('users.role', $data->id) }}" data-confirm="true" data-icon="question"
                                    data-caption="Apakah anda ingin mengubah role {{ $data->email }} {{ $data->role == 'admin' ? 'menjadi Customer biasa?' : 'menjadi Admin?' }}"
                                    class="flex justify-center items-center gap-1 border rounded-md py-1 px-2 text-xs bg-blue-200 text-blue-700 font-semibold border-blue-500 hover:opacity-85">
                                ...
                                <a href="{{ route('users.destroy', $data->id) }}" data-confirm-delete2="true"
                                    data-caption="Apakah anda yakin ingin menonaktifkan {{ $data->email }}?"

    ...
    <div class="max-w-screen-xl mx-auto">
        ...
        <div class=" border-t border-gray-600 p-3 grid grid-cols-1 md:grid-cols-2 gap-8">
                        ...
                        <div x-show="open" x-transition @click.self="open = false" @keydown.escape.window="open = false"
                            ...
                            <div class="relative max-w-4xl w-full flex flex-col justify-center items-center">
                                <img :src="imageUrl" alt="Preview"
                                    class="max-h-[85vh] max-w-full rounded-lg shadow-lg border">
                            </div>
                        </div>
                    </template>
                @endif
            ...
            @if ($data->status == 'ditinjau')
                <a href="{{ route('pengembalian.destroy', [$data->transaksi->kodeTrx, $data->id]) }}"
                    data-confirm-delete="true"
                    class="col-span-2 border-2 border-red-500 text-red-500 rounded-lg py-2 font-semibold hover:bg-red-500 hover:text-white duration-200 text-center">Batalkan
                    Pengajuan</a>
            @endif
        </div>
    </div>
